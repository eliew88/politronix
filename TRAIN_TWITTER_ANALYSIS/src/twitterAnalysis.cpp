...
TwitterAnalysis::TwitterAnalysis(){
    tu = new TextUtils;
    positive_seed_words = {"good", "great", "happy", "excited", ":)", "<3", "love", "like", ":-)"};
    negative_seed_words = {"bad", "terrible", "sad", ":(", "hate", "dislike", ":-("};
}
...
void TwitterAnalysis::loop_through_word_pairs(string outputfile, double threshold_corr) {
    ...
    // find correlations with positive seed words
    for (string word1 : positive_seed_words) {
        for (const auto &key_value : word_counts) {
            string word2 = key_value.first;
            double correlation = calculate_correlation(word1, word2);
            word_pairs++;
            if (correlation != 0) { //for debugging
                cout << word1 << " " << word2 << " " << correlation << endl;
            } 
    ...
    // find correlations with negative seed words
    for (string word1 : negative_seed_words) {
        for (const auto &key_value : word_counts) {
            string word2 = key_value.first;
            double correlation = calculate_correlation(word1, word2);
            word_pairs++;
